...
class TeacherController extends Controller
{
    ...
    public function reportingList(Request $request)
    {
        ...
        if($class_id == 'all' || $class_id != ""){
            $reportings->where('class_id', $class_id);
        }
...
//      $reportings = $reportings->join('enrollments', 'reportings.id', '=', 'enrollments.user_id')->select('enrollments.*')->paginate(10);
        ...
        return view('teacher.reporting.report_list', compact('reportings', 'search', 'classes', 'class_id', 'section_id'));
    }
...
//        dd($data);
        ...
        if($request->hasFile('photo'))
        {
            ...
            foreach($request->file('photo') as $image)
            {
                ...
                $filename = md5(uniqid(rand(), true)) . $filename;
        ...
        return redirect()->back()->with('message','You have successfully add Report.');
    }

    public function reportingCardGenerate($id)
    {
        $search = $request['search'] ?? "";
        $class_id = $request['class_id'] ?? "";
        $section_id = $request['section_id'] ?? "";
        ...
        $reportings = Reporting::where('id', $id)->first();
        ...
        return view('teacher.report_card', ['reportings' => $reportings]);
    }
    ...
    public function reportingUpdate(Request $request, $id)
    {
        ...
        if($request->hasFile('photo'))
        {
            $existingPhotos = json_decode($reporting['photo'], true); // Decode the JSON string to an array
            $newPhotos = [];

            foreach ($request->file('photo') as $image) {
                $filename = md5(uniqid(rand(), true)) . $image->getClientOriginalName();
                $image->move(public_path('assets/uploads/reporting-images/'), $filename);
                $newPhotos[] = $filename;
            }

            $combinedPhotos = array_merge($existingPhotos, $newPhotos); // Merge existing and new photos

            $reporting->update([
                'photo' => json_encode($combinedPhotos), // Encode the combined photos back to JSON
            ]);
    ...
    public function reportingDelete($id)
    {
