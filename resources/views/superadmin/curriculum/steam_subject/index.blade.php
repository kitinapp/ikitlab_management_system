      ...
      <div class="col-md-12">
        ...
        >
          <div class="d-flex flex-column">
            <h4>{{ get_phrase('STEAM Subjects') }}</h4>
            <ul class="d-flex align-items-center eBreadcrumb-2">
              <li><a href="#">{{ get_phrase('Home') }}</a></li>
              <li><a href="#">{{ get_phrase('STEAM') }}</a></li>
              <li><a href="#">{{ get_phrase('Subject') }}</a></li>
            </ul>
          </div>
          <div class="export-btn-area">
            <a href="javascript:;" class="export_btn" onclick="rightModal('{{ route('superadmin.steam_subject.open_modal') }}', '{{ get_phrase('Create STEAM Subject') }}')">{{ get_phrase('Create STEAM Subject') }}</a>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="col-md-12">
        <div class="eSection-wrap">
            <div class="search-filter-area d-flex justify-content-md-between justify-content-center align-items-center flex-wrap gr-15">
              ...
              <!-- Export Button -->
              @if(count($steam_subject_lists) > 0)
            ...
            <div class="table-responsive tScrollFix pb-2">
                  <table class="table eTable">
                    ...
                    <tbody>
                        @foreach($steam_subject_lists as $key => $steam_subject_list)
                             <tr>
                                <td>{{ $steam_subject_lists->firstItem() + $key }}</td>
                                 <td>{{ $steam_subject_list->steam->title }}</td>
                                <td>{{ $steam_subject_list->title }}</td>
                                <td class="text-start">
...
{{--                                            <a class="dropdown-item" href="javascript:;" onclick="rightModal('{{ route('superadmin.edit.section', ['id' => $steam_list->id]) }}', '{{ get_phrase('Edit Section') }}')">{{ get_phrase('Edit Section') }}</a>--}}
                                          ...
                                          <li>
                                            <a class="dropdown-item" href="javascript:;" onclick="rightModal('{{ route('superadmin.edit.steam_subject', ['id' => $steam_subject_list->id]) }}', '{{ get_phrase('Edit STEAM Subject') }}')">{{ get_phrase('Edit STEAM Subject') }}</a>
                                          </li>
                                          <li>
                                            <a class="dropdown-item" href="javascript:;" onclick="confirmModal('{{ route('superadmin.steam_subject.delete', ['id' => $steam_subject_list->id]) }}', 'undefined');">{{ get_phrase('Delete') }}</a>
                                          </li>
                                        </ul>
                                    </div>
                                </td>
                ...
                {!! $steam_subject_lists->appends(request()->all())->links() !!}
            </div>
            @else
