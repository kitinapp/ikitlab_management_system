        ...
        >
          <div class="d-flex flex-column">
            ...
            <ul class="d-flex align-items-center eBreadcrumb-2">
              ...
              <li><a href="#">{{ get_phrase('STEAM List') }}</a></li>
            </ul>
          </div>
          <div class="export-btn-area">
            <a href="javascript:;" class="export_btn" onclick="rightModal('{{ route('superadmin.steam.open_modal') }}', '{{ get_phrase('Create STEAM') }}')">{{ get_phrase('Create STEAM') }}</a>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="col-7 offset-md-2">
        <div class="eSection-wrap">
            <div class="search-filter-area d-flex justify-content-md-between justify-content-center align-items-center flex-wrap gr-15">
              <form action="{{ route('superadmin.steam_list') }}">
                <div
            ...
            <div class="table-responsive tScrollFix pb-2">
                  <table class="table eTable">
                    ...
                    <tbody>
                        @foreach($steam_lists as $key => $steam_list)
                             <tr>
                                <td>{{ $steam_lists->firstItem() + $key }}</td>
                                <td>{{ $steam_list->title }}</td>
                                <td class="text-start">
...
{{--                                            <a class="dropdown-item" href="javascript:;" onclick="rightModal('{{ route('superadmin.edit.section', ['id' => $steam_list->id]) }}', '{{ get_phrase('Edit Section') }}')">{{ get_phrase('Edit Section') }}</a>--}}
                                          ...
                                          <li>
                                            <a class="dropdown-item" href="javascript:;" onclick="rightModal('{{ route('superadmin.edit.steam', ['id' => $steam_list->id]) }}', '{{ get_phrase('Edit STEAM') }}')">{{ get_phrase('Edit STEAM') }}</a>
                                          </li>
                                          <li>
                                            <a class="dropdown-item" href="javascript:;" onclick="confirmModal('{{ route('superadmin.steam.delete', ['id' => $steam_list->id]) }}', 'undefined');">{{ get_phrase('Delete') }}</a>
                                          </li>
                                        </ul>
                                    </div>
                                </td>
                ...
                {!! $steam_lists->appends(request()->all())->links() !!}
            </div>
            @else
...
<script type="text/javascript">
  ...
  function Export() {
    ...
    var opt = {
      margin:       1,
      filename:     'steam_list_{{ date("y-m-d") }}.pdf',
